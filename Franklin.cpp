...
int main(int argc, char** argv) {
	...
	while(true) // loop to continue until a leader is elected
	{
		if (p.getStatus() == true) { // If an active node			
			...
			if (min(left_id, right_id) > my_id) // compare IDs to see if I am the smallest process in my *active* neighbourhood
			{
				p.setStatus(false); // enter relay mode if my ID is the smallest of the three
			}
			else if (min(left_id, right_id) == my_id) // If I am my own left and right neighbour i.e. all others have become passive by now then:
			{
